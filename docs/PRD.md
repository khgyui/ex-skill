...
│   ├── voice_input.py         # 🆕 语音录入（麦克风→文字，STT）
│   └── voice_output.py        # 🆕 音色输出（文字→语音合成，TTS）
│
...
- [ ] 截图 OCR 支持
- [x] `tools/voice_input.py` 语音录入（麦克风→文字，支持 edge-stt / whisper-local / whisper-api）
- [x] `tools/voice_output.py` 音色输出（文字→语音合成，基于 edge-tts 音色映射）
- [x] 音色注册功能（分析语音样本 → 匹配内置音色 → 保存配置）
- [x] 语音交互集成到 SKILL.md 和各 ex 的对话流程
